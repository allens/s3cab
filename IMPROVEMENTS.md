...
Three of the five candidates shipped — the dead lstat cache (#67), the snapshot-grammar
consolidation (candidate A, #69, which also retired "manifest", #68), and the env front door
(candidate B): `auth.mjs`'s env-layering split out into `env.mjs`, and a `prepareRemoteSet`
front door the four set-family commands call instead of hand-coding `resolveRemoteSet` +
`loadEnv` — consolidated, not type-enforced ([ADR-0022](docs/adr/0022-prepare-remote-set-front-door.md);
scope settled to the set family only, the bucket family keeps its inline `loadEnv`). The
remaining two, by strength:
