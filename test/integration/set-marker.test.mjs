...
describe("set marker (real bucket)", () => {
  ...
  it("writeRemoteInfo re-stamps the marker (the reconnect takeover)", async () => {
    ...
    try {
      ...
      // Re-stamp OWNER, preserve CREATED (what reconnect does).
  ...
  it("pushSetConfig/readSetConfig round-trips dirs and an optional exclude", async () => {
    ...
    try {
      ...
      // (mirror local), so reconnect can't resurrect a removed exclude.txt.
