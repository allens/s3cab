...
full design in [docs/specs/backup.md](../specs/backup.md). `sets` requires `--bucket` and
...
`s3cab sets` requires `--bucket` when creating: a set is bound to its bucket at creation, and
...
they were never actually configured for backup, and has to go back to `sets` to bind a bucket
...
collision check lives in exactly one place (`sets`) instead of firing at every "first remote
