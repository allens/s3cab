...
- [0068](0068-network-retries-above-the-sdk.md) — Transport failures are retried *above* the SDK on a 120 s time window, because the SDK's retry budget is a client-wide token bucket that gives a ≥512 MB file only ~0.2 s of tolerance and makes `maxAttempts` inert; throttling/5xx keep stock SDK behaviour *(accepted & implemented; completes 0065's open items, extends 0037's relay. Amended 2026-09-13 by 0091: the window runs from the first failure, not from the request)*
- [0091](0091-idle-connection-bound-and-retry-window-origin.md) — A pooled connection idle past 10 s is destroyed rather than handed out again (plain `httpAgent`/`httpsAgent` options, so the handler stays unimported), and 0068's retry window is measured from the first failure. Both from one incident, and the probe sorted them: S3 closes an idle connection after ~5 s and Node hears it, so reuse past that doesn't happen on a healthy link — the bound is a backstop for the close going *unheard*, which costs no reuse and is not reproducible on demand, while the window origin is what actually decided the run (a slow-to-discover failure spent the window on discovery and bought one retry). Shortening 0065's 30 s timeout is rejected (a stalled local read would kill a healthy upload); the "your network connection dropped" wording is rejected with it, since s3cab only knows the request went unanswered *(accepted & implemented; corrects the silent-failure path of 0065 and 0068, measured by `scripts/idle-socket-probe.mjs`)*
