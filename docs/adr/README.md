...
- [0087](0087-deletion-record-suffix-on-collision.md) — A deletion record whose minute-precision name is taken retries under `<name>-2`, `-3`, … instead of failing; the PUT stays conditional (the suffix disambiguates a file, never licenses an overwrite) and the `S3CAB_DEBUG` overwrite escape is removed with the collision it worked around. 0064 inherited the snapshot's refusal along with its name grammar, but refusal buys a snapshot's "an accidental double-run can't destroy history" and buys a record nothing — two deletes are two real events, and both must be recorded. Readers take `<timestamp>[-<n>].tsv` *(accepted & implemented; amends 0064's naming bullet; the snapshot same-minute race is deliberately untouched — same mechanism, opposite answer)*
