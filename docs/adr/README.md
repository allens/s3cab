...
- [0080](0080-online-only-files-skipped.md) — A dehydrated cloud placeholder (Windows Files On-Demand: OneDrive, Dropbox, Google Drive) is left online and reported as an `Online-Only File` skip, so a first backup stops silently hydrating a whole cloud account onto the local disk. Detected by `size >= 4096 && blocks === 0` on the `lstat` `fileProps` already takes — the 4KB floor clears NTFS's MFT-resident small files — checked *after* the baseline reuse, and confined to Windows because a fully sparse ext4 file is the identical shape; `--include-online-only` opts in *(accepted & implemented; routes through 0078's skip channel, applies 0030/0012, macOS left open)*
