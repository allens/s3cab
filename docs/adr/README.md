...
- [0079](0079-exclusion-review-from-the-walk.md) — `tree --excluded` answers "what are my patterns dropping?" from a **live walk**, not from a snapshot's `#EXCLUDED` rows (which are written but never read back) — so an `exclude.txt` edit can be checked by re-running. One flag, not two: the pattern rides on every row, making "why is *this* excluded?" a `grep`. stdout is `<path>` TAB `<pattern>`, stderr a count per pattern *(accepted & implemented; applies 0010/0043, uses the records 0028 already hands back)*
