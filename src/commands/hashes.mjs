 ...
 * s3cab's composition medium — the same enumeration `verify`/`cleanup` diff and a
 * first `backup` LISTs, reproducible by hand for maintenance (docs/design/backup.md).
 * That stream is the result, returned for the render layer (ADR-0043) and rendered
 * one hash per line; a file is produced with plain shell redirection
 * (`s3cab hashes <bucket> > file`), so the command does no I/O of its own (the old
 * `--output` flag was pure redundancy with `>` — dropped, ADR-0006).
 *
