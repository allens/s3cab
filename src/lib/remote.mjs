 ...
 *
 * Requires the target bucket's env loaded first — the user env at the entry
 * point, plus the set's env via `loadSet` (env.mjs, ADR-0022).
 ...
 *
 * Requires the target bucket's env loaded first — the user env at the entry
 * point, plus the set's env via `loadSet` (env.mjs, ADR-0022). (`setup --inherit`
 * is the caller here; its user env comes from the entry point too.)
 * @param {string} bucket - The repository's S3 bucket
 * @param {string} set - The set's name (its whole identity, ADR-0024)
