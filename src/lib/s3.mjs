...
/**
 * Open a readable over an S3 object — the SDK's `GetObject` `Body`, which is
 * already a Node `Readable`, handed back with no wrapper. Callers feed it
 * straight into `pipeline`/`downloadToFile`, so error propagation and teardown
 * come from those primitives (a mid-download failure rejects the pipeline; an
 * aborted consumer destroys the source) — there is nothing to bridge here.
 * @param {string} uri - The `s3://bucket/key` URI of the object.
 * @returns {Promise<Readable>}
 */
export async function openObjectBody(uri) {
 ...
 *
 * The source is handed in (typically `await openObjectBody(uri)`) rather than
 ...
 * @param {import("node:stream").Readable} source - The object's byte stream (typically `await openObjectBody(uri)`)
 ...
 * the string twin of `openObjectBody` for the marker / config files (`info`,
