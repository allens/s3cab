...
// presence of `info` is how the collision check and `reconnect` learn a name is
 ...
 * `reconnect` (which preserves `CREATED`).
 * @param {string} bucket
 * @param {string} set
...
/**
 * Overwrite a set's remote `info` marker — used by `reconnect` to re-stamp
 * `OWNER` to the reconnecting machine (the caller preserves `CREATED`). A plain
 ...
 * leave one behind for `reconnect` to resurrect. Plain overwrites — the caller
 * owns the set (it won the claim or reconnected to it).
 * @param {string} bucket
 * @param {string} set
 * @param {object} config
...
/**
 * Read a set's published config back from its remote marker — what `reconnect`
 ...
 * segments under `sets/`. The discovery aid in `reconnect`'s "no such set"
